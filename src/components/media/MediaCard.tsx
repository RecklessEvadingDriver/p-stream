  ...
  return (
    <Flare.Base
      className={`group -m-[0.705em] rounded-xl bg-background-main transition-all duration-300 ease-out focus:relative focus:z-10 ${
        canLink ? "hover:bg-mediaCard-hoverBackground hover:shadow-xl tabbable" : ""
      } ${closable ? "jiggle" : ""}`}
    ...
    >
      ...
      />
      <Flare.Child
        className={`pointer-events-auto relative mb-2 p-[0.4em] transition-all duration-300 ease-out ${
          canLink ? "group-hover:scale-95" : "opacity-60"
        }`}
      >
        <div
          className={classNames(
            "relative mb-4 pb-[150%] w-full overflow-hidden rounded-xl bg-mediaCard-hoverBackground bg-cover bg-center transition-all duration-300 ease-out",
            {
              "group-hover:rounded-lg group-hover:scale-105": canLink,
            },
          )}
          style={{
